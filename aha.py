...
def prepare_model_inputs(
    query_ids: List[List[int]], response_ids: List[List[int]]
) -> dict:
    ...
    batch_inputs = {"input_ids": [], "attention_mask": [], "labels": []}
...
def compute_pg_loss(
    ...
    with torch.no_grad():
        ref_logps = compute_token_log_probs(
            reference_model, batch_model_inputs, temperature
        )
...
def evaluate_on_test_set(
    sglang_engine, test_dataset, tokenizer, EOS_TOKEN, SAMPLING_PARAMS
):
    ...
    generations = sglang_engine.generate(
    ...
    for i, datum in enumerate(test_dataset):
        ...
        equation_reward = equation_reward_func(response, datum["target"], datum["nums"])
        format_reward = format_reward_func(response, EOS_TOKEN)
...
def training_episode_generator(
    generations, batch_samples, tokenizer, EOS_TOKEN, EOS_TOKEN_ID, ROLLOUTS_PER_EPISODE
):
    """
    ...
    # Process responses and calculate rewards
    groups = [
        list(range(i, i + ROLLOUTS_PER_EPISODE))
        for i in range(0, len(generations), ROLLOUTS_PER_EPISODE)
    ]
    ...
    for datum, group_indices in zip(batch_samples, groups):
        ...
        finish_reasons = [
            generations[i]["meta_info"]["finish_reason"]["type"] for i in group_indices
        ]
        ...
        for response in responses:
            equation_reward = equation_reward_func(
                response, datum["target"], datum["nums"]
            )
            format_reward = format_reward_func(response, EOS_TOKEN)
...
def main():
    ...
    dataset = dataset.map(
        lambda example: generate_r1_prompt(
            example["nums"], example["target"], tokenizer
        ),
        num_proc=6,
    )
    ...
    for iteration in trange(NUM_ITERATIONS):
        ...
        batch_indices = np.random.choice(
            len(train_dataset), size=num_ds_samples, replace=False
        )
        ...
        success, error = sglang_engine.update_weights_from_tensor(
        ...
        print(f"Time taken to generate {len(generations)} responses: {time.time() - gen_time} seconds")
