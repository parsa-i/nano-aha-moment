...
def preprocess_example(
    ...
    # Sample a thinking budget between THINKING_BUDGET_MIN and THINKING_BUDGET_MAX
    thinking_budget = random.randint(THINKING_BUDGET_MIN, THINKING_BUDGET_MAX)
    # Round to the nearest multiple of 10
    thinking_budget = round(thinking_budget / 10) * 10
    ...
    prefix = [
        {"role": "system", "content": SYSTEM_MESSAGE},
        {
            ...
            "content": PROMPT_TEMPLATE.format(
                numbers=numbers, target=target, thinking_budget=thinking_budget
            ),
        },
    ...
    return {"prompt": prompt, "input_ids": input_ids, "thinking_budget": thinking_budget}
...
def thinking_budget_reward_func(completion: str, thinking_budget: int, tokenizer: AutoTokenizer) -> float:
    """
    Evaluates completion based on the length of the <think> tags
    """
    tokens = tokenizer.encode(completion, add_special_tokens=False)
    if len(tokens) > thinking_budget:
        return 0.0
    else:
        return 1.0
...
def compute_reward(
    completion: str, sample: Dict[str, Any], EOS_TOKEN: str, tokenizer: AutoTokenizer
) -> Tuple[float, Dict[str, float]]:
    nums = sample["nums"]
    target = sample["target"]
    thinking_budget = sample["thinking_budget"]
    ...
    thinking_budget_reward = thinking_budget_reward_func(
        completion=completion, thinking_budget=thinking_budget, tokenizer=tokenizer
    )

    reward = format_reward + equation_reward + thinking_budget_reward

    metrics = {
        "format_reward": format_reward,
        "equation_reward": equation_reward,
        "thinking_budget_reward": thinking_budget_reward,
    }
...
def create_training_episodes(
    ...
    for sample, group_indices in zip(samples, groups):
        ...
        rewards_and_metrics = [
            compute_reward(resp, sample, EOS_TOKEN, tokenizer) for resp in responses
        ]
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Minimum thinking budget for training",
    )
    parser.add_argument(
        "--thinking_budget_max",
        type=int,
        default=1024,
        help="Maximum thinking budget for training",
    )
    ...
    THINKING_BUDGET_MIN = args.thinking_budget_min
    THINKING_BUDGET_MAX = args.thinking_budget_max
    ...
    RUN_NAME = (
        f"{model_name_short}_temp{TEMPERATURE}_kl{KL_COEFFICIENT}_lr{LEARNING_RATE}_"
        f"thinking_budget_min{THINKING_BUDGET_MIN}_thinking_budget_max{THINKING_BUDGET_MAX}"
    )
    ...
    PROMPT_TEMPLATE = "Using the numbers {numbers}, create an equation that equals {target}. You can use basic arithmetic operations (+, -, *, /) and each number can only be used once. Show your work in <think> </think> tags. And return the final equation and answer in <answer> </answer> tags, for example <answer>(1 + 2) / (3 * 5)</answer>. You have a strict thinking budget of {thinking_budget} tokens. No more than {thinking_budget} tokens should be used in <think> tags. Use the best of your ability to solve the problem within the thinking budget."
        ...
        if (iteration + 1) % 25 == 0:
            ...
            eval_stats = evaluate_on_test_set(
                ...
                reward_func=lambda completion, sample: compute_reward(
                    completion, sample, EOS_TOKEN, tokenizer
                ),
            )
